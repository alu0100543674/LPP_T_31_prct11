module Test
    ...
    class Interfaz
        ...
        attr_accessor :contador_acertadas , :cont_erroneas, :arr_respuesta
    
        def initialize(arr_respuesta)
	        @arr_respuesta= arr_respuesta
        ...
        def hacer_examen(respuestas)
	        cont = 0
	        num_preguntas = 0
	        while num_preguntas <  5
	            if (@arr_respuesta[cont] == respuestas[cont])
		            @contador_acertadas = @contador_acertadas + 1
	            else
		            @cont_erroneas = @cont_erroneas + 1
	            end
	            cont = cont + 1
	            num_preguntas += 1
            end

	        if (@contador_acertadas > @cont_erroneas)
	            return true
	        else
	            return false
	        end       
    ...
    class Examen
        ...
        def invertir(lista)
	        @lista_invertida = Test::ListaEnlazada.new()
	        lista.reverse_each do |q|
	            @lista_invertida.push(q)
	        end
	        return @lista_invertida
        end
      
        def invertir_funcional(lista)
            ar = Array.new
	        ar.push(lista)
	        invertir = -> { reverse }
	        ar = lista.instance_exec(&invertir)
	        return ar
        end
    end
end 
